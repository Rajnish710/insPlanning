...
while($row = sqlsrv_fetch_array($run, SQLSRV_FETCH_ASSOC)){
	...
	$sql1 = "SELECT per as planTol, CEILING(".$row['CQty']." * (1 + per * 0.01)) AS newCutLength FROM [PlanningSys].[master].[qtyTol] WHERE ".$row['CQty']." BETWEEN lenFrom AND lenTo AND minus = '".$row['TolM']."' AND plus = '".$row['TolP']."'";
    ...
    if ($isNoPrintOnCore || $isBandM) {
        ...
        $sql3 = "SELECT JobNo,Color,1 as ColorCount,ROW_NUMBER() OVER (PARTITION BY PairNo ORDER BY Color) AS SrNo
                from (SELECT JobNo, color_1 AS Color, '1' as PairNo FROM OrdColorIns 
                where JobNo = '$job' 
                UNION ALL 
                SELECT JobNo, color_2 AS Color, '2' as PairNo FROM OrdColorIns 
                where JobNo = '$job' 
                UNION ALL 
                SELECT JobNo, color_3 AS Color, '3' as PairNo FROM OrdColorIns 
                where JobNo = '$job' ) as tbl 
                where Color IS NOT NULL 
                order by PairNo,Color";
        ...
        if ($count > 0) {
            ...
            $sqlin = "INSERT INTO [PlanningSys].[instru].[data] ([DeliveryDate] ,[JobNo] ,[Core] ,[CP] ,[Sqmm], [isMica],[OrderQty] ,[TolM] ,[TolP], [planTol],[OrdCutLength] ,[ord_NoofDrums] ,[CondGrade] ,[CondType] ,[InsuType] ,[NoOfStr] ,[StrDia] ,[PlanCutLen] ,[plan_NoofDrums], [multiplyDrums],[InsuColor] , [PairNo],[PlanningDate] ,[RequiredDate] ,[ordID] ,[cableCat] ,[enqNo] ,[valueAmt] ,[LdPer] ,[LdTerm] ,[createdAt] ,[createdBy],[drawingCap],[tinningCap],[bunchCap],[insuCap],[isBandM],[isNoPrintOnCore]) VALUES ('".$row['SiteDate']."', '".$row['JobNo']."', '".$row['Core']."', '".$row['CorePair']."', '".$row['SQMM']."', '".$isMica."', '".$row['Qty']."', '".$row['TolM']."', '".$row['TolP']."', '".$row['planTol']."', '".$row['CQty']."', '".$row['NoofDrum']."', '".$row['condgrd']."', '".$row['Condtypep']."', '".$row['insGrade']."', '".$row['noOfStr']."', '".$row['StrDia']."', '".$row['planLen']."', '".$row['NoofDrum']."', '".$row3['ColorCount']."','".$row3['Color']."','".$row3['SrNo']."', '".date('Y-m-d')."', '".$insuReqDate."', '".$row['ordid']."', '".$row['CableCat']."', '".$row['OfferNo']."', '".$offerVal."', '".$ldWeekPer."', '".$row['LDASPER']."', '".date('Y-m-d H:i:s')."', '','$drawingCap','$tinningCap','$bunchCap','$insuCap', '".$isBandM."', '".$isNoPrintOnCore."')";
            ...
            for ($i=1; $i <= $PTQArr[$row['CorePair']]; $i++) { 
                for ($j=1; $j <= intval($row['Core']) ; $j++) { 
                    $sqlin = "INSERT INTO [PlanningSys].[instru].[data] ([DeliveryDate] ,[JobNo] ,[Core] ,[CP] ,[Sqmm], [isMica],[OrderQty] ,[TolM] ,[TolP], [planTol],[OrdCutLength] ,[ord_NoofDrums] ,[CondGrade] ,[CondType] ,[InsuType] ,[NoOfStr] ,[StrDia] ,[PlanCutLen] ,[plan_NoofDrums], [multiplyDrums] ,[InsuColor], [PairNo],[PlanningDate] ,[RequiredDate] ,[ordID] ,[cableCat] ,[enqNo] ,[valueAmt] ,[LdPer] ,[LdTerm] ,[createdAt] ,[createdBy],[drawingCap],[tinningCap],[bunchCap],[insuCap],[isBandM],[isNoPrintOnCore]) VALUES ('".$row['SiteDate']."', '".$row['JobNo']."', '".$row['Core']."', '".$row['CorePair']."', '".$row['SQMM']."', '".$isMica."', '".$row['Qty']."', '".$row['TolM']."', '".$row['TolP']."', '".$row['planTol']."', '".$row['CQty']."', '".$row['NoofDrum']."', '".$row['condgrd']."', '".$row['Condtypep']."', '".$row['insGrade']."', '".$row['noOfStr']."', '".$row['StrDia']."', '".$row['planLen']."', '".$row['NoofDrum']."', '1','".$row['Col'.$i]."', '$j', '".date('Y-m-d')."', '".$insuReqDate."', '".$row['ordid']."', '".$row['CableCat']."', '".$row['OfferNo']."', '".$offerVal."', '".$ldWeekPer."', '".$row['LDASPER']."', '".date('Y-m-d H:i:s')."', '','$drawingCap','$tinningCap','$bunchCap','$insuCap', '".$isBandM."', '".$isNoPrintOnCore."')";
                    $runin = sqlsrv_query($conn,$sqlin);    
                        if ($runin == false) {
                            error_log($logMessage.$row['JobNo'], 3, "error.log");
                        }   
    ...
    } else { // without numbering pair
        $sql3 = "SELECT JobNo,Color,COUNT(Color) AS ColorCount,PairNo from (SELECT JobNo, color_1 AS Color, '1' as PairNo FROM OrdColorIns where JobNo = '".$row['JobNo']."' UNION ALL SELECT JobNo, color_2 AS Color, '2' as PairNo FROM OrdColorIns where JobNo = '".$row['JobNo']."' UNION ALL SELECT JobNo, color_3 AS Color, '3' as PairNo FROM OrdColorIns where JobNo = '".$row['JobNo']."' ) as tbl where Color IS NOT NULL Group by JobNo,color,PairNo";
        $result = sqlsrv_query($conn,$sql3,$params,$options);
        $count = sqlsrv_num_rows($result);
        if ($count > 0) {
            $run3 = sqlsrv_query($conn,$sql3);
            ...
            $sqlin = "INSERT INTO [PlanningSys].[instru].[data] ([DeliveryDate] ,[JobNo] ,[Core] ,[CP] ,[Sqmm], [isMica],[OrderQty] ,[TolM] ,[TolP], [planTol],[OrdCutLength] ,[ord_NoofDrums] ,[CondGrade] ,[CondType] ,[InsuType] ,[NoOfStr] ,[StrDia] ,[PlanCutLen] ,[plan_NoofDrums], [multiplyDrums],[InsuColor] , [PairNo],[PlanningDate] ,[RequiredDate] ,[ordID] ,[cableCat] ,[enqNo] ,[valueAmt] ,[LdPer] ,[LdTerm] ,[createdAt] ,[createdBy],[drawingCap],[tinningCap],[bunchCap],[insuCap],[isBandM],[isNoPrintOnCore]) VALUES ('".$row['SiteDate']."', '".$row['JobNo']."', '".$row['Core']."', '".$row['CorePair']."', '".$row['SQMM']."', '".$isMica."', '".$row['Qty']."', '".$row['TolM']."', '".$row['TolP']."', '".$row['planTol']."', '".$row['CQty']."', '".$row['NoofDrum']."', '".$row['condgrd']."', '".$row['Condtypep']."', '".$row['insGrade']."', '".$row['noOfStr']."', '".$row['StrDia']."', '".$row['planLen']."', '".$row['NoofDrum']."', '".$row3['ColorCount']."','".$row3['Color']."','0', '".date('Y-m-d')."', '".$insuReqDate."', '".$row['ordid']."', '".$row['CableCat']."', '".$row['OfferNo']."', '".$offerVal."', '".$ldWeekPer."', '".$row['LDASPER']."', '".date('Y-m-d H:i:s')."', '','$drawingCap','$tinningCap','$bunchCap','$insuCap', '".$isBandM."', '".$isNoPrintOnCore."')";
            $runin = sqlsrv_query($conn,$sqlin);    
            if ($runin == false) {
                error_log($logMessage.$row['JobNo'], 3, "error.log");
            }
            ...
            for ($i=1; $i <= $PTQArr[$row['CorePair']]; $i++) { 
                $sqlin = "INSERT INTO [PlanningSys].[instru].[data] ([DeliveryDate] ,[JobNo] ,[Core] ,[CP] ,[Sqmm], [isMica],[OrderQty] ,[TolM] ,[TolP], [planTol],[OrdCutLength] ,[ord_NoofDrums] ,[CondGrade] ,[CondType] ,[InsuType] ,[NoOfStr] ,[StrDia] ,[PlanCutLen] ,[plan_NoofDrums], [multiplyDrums] ,[InsuColor], [PairNo],[PlanningDate] ,[RequiredDate] ,[ordID] ,[cableCat] ,[enqNo] ,[valueAmt] ,[LdPer] ,[LdTerm] ,[createdAt] ,[createdBy],[drawingCap],[tinningCap],[bunchCap],[insuCap],[isBandM],[isNoPrintOnCore]) VALUES ('".$row['SiteDate']."', '".$row['JobNo']."', '".$row['Core']."', '".$row['CorePair']."', '".$row['SQMM']."', '".$isMica."', '".$row['Qty']."', '".$row['TolM']."', '".$row['TolP']."', '".$row['planTol']."', '".$row['CQty']."', '".$row['NoofDrum']."', '".$row['condgrd']."', '".$row['Condtypep']."', '".$row['insGrade']."', '".$row['noOfStr']."', '".$row['StrDia']."', '".$row['planLen']."', '".$row['NoofDrum']."', '".$row['Core']."','".$row['Col'.$i]."', '0', '".date('Y-m-d')."', '".$insuReqDate."', '".$row['ordid']."', '".$row['CableCat']."', '".$row['OfferNo']."', '".$offerVal."', '".$ldWeekPer."', '".$row['LDASPER']."', '".date('Y-m-d H:i:s')."', '','$drawingCap','$tinningCap','$bunchCap','$insuCap', '".$isBandM."', '".$isNoPrintOnCore."')";
            $runin = sqlsrv_query($conn,$sqlin);    
            if ($runin == false) {
                error_log($logMessage.$row['JobNo'], 3, "error.log");
            }   
