...
SELECT
    d.id,
    DENSE_RANK() OVER (ORDER BY d.isBandM, d.isMica, d.CondType, d.InsuType, CONCAT(d.NoOfStr, '/', d.StrDia), d.InsuColor) AS srNo,
    d.Sqmm,
    d.JobNo,
...
foreach ($finalArr as $key => $row) {
   if ($row['DrumNo'] != $prevDrumNo) {
    ...
    $returnArr[] = array_merge($row,
                               [
                                'DrumNo' => $row['isBandM'].$dr,
                                'cls' => $lastChangeRow,
                                'chk' => '',
                                 'totalCore' => '=O'.($key+1).'*P'.($key+1).'',
                                 'PlanCutLen' => '=M'.($key+1).'*(1+N'.($key+1).'*0.01)'
                               ]
                              );   
}
