...
{ type: 'hidden', name: 'isBandM',title: 'isBandM', width:'90px' }, //V
{ type: 'calendar', name: 'insuStartDate',title: 'Insu Start Date', width:'90px', options: {  format: 'DD-MM-YY' } }, //W
                    ...
                    let missingGrade = [];
                        data.forEach((e,i) => {
                            if (e['chk'] == 'true') {
                                // check color code 
                    const is2pColorCode = 
                        ['InsuColor3', 'InsuColor4'].every(key => {
                            const val = e?.[key];
                            return val != null && val.toString().trim() !== '' && val.toString().trim() !== '0';
                        });
